...
async def login(page, use_duo=True):
    try:
        ...
        print_red(f"Error during login: {e}")
...
async def detect_garage_parking_permits(page):
    try:
        ...
        print_red(f"Error detecting garage parking permits: {e}")
...
async def run(playwright, headless, reload_time=60):
    try:
        ...
        try:
            ...
            print_red(f"Cannot go to the login page: {e}")
        ...
        print_red(f"Error in run function: {e}")
...
async def main():
    try:
        ...
        print_red(f"Error in main function: {e}")
...
def send_email(garage_type):
    try:
        ...
        print_red(f"Error sending email: {e}")
